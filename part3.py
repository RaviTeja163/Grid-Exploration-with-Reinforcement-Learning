...
def generate_graph(time_points, mean_rewards):
    ...
    df1.to_csv('part3_data.csv', header=True, index=False)
...
def play(world, e, span_time):
    ...
    mean_rewards = []
    t_interval = -1
    if max(world.row, world.col) <= 10:
        n = 100
    else:
        n = 5000
    print(n)
    while run_time < span_time:
        world.run_episode(e=n/(n+episodes))
        # world.run_episode(e)
        episodes += 1
...
def main():
    if len(sys.argv) != 6:
        print("Required format: part3.py <filename> <reward> <gamma> <time to learn> <movement probability>")
        exit(1)
    else:
        file = sys.argv[1]
